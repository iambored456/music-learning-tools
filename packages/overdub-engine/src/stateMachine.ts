...
function touchProject(state: OverdubEngineState): void {
  state.project.updatedAt = nowIso();
}

function transitionToIdleIfEmpty(state: OverdubEngineState): void {
  state.mode = hasCommittedTakes(state.project) ? 'overdubArmed' : 'idle';
}

function findLayer(state: OverdubEngineState, layerId: string): OverdubLayer | null {
  return state.project.layers.find((layer) => layer.id === layerId) ?? null;
}

function selectDefaultLayerId(state: OverdubEngineState): string | null {
  const lastLayer = state.project.layers[state.project.layers.length - 1];
  return lastLayer?.id ?? null;
}

function normalizePhrase(phrase: PhraseSettings): PhraseSettings {
  return {
...
function trimLayerTakes(layer: OverdubLayer, maxTakesPerLayer: number): void {
  if (layer.takes.length <= maxTakesPerLayer) return;
...
function sanitizeTake(take: OverdubTake, layerId: string): OverdubTake {
  return {
    ...take,
    layerId,
...
function isModeOneOf(mode: OverdubEngineState['mode'], valid: OverdubEngineState['mode'][]): boolean {
  return valid.includes(mode);
}

function createInitialState(config: OverdubEngineConfig): OverdubEngineState {
  return {
    mode: 'idle',
...
export function createOverdubEngine(partialConfig: Partial<OverdubEngineConfig> = {}): OverdubEngine {
  let config: OverdubEngineConfig = {
    ...DEFAULT_ENGINE_CONFIG,
    ...partialConfig,
    defaultPhrase: {
  ...
  let state: OverdubEngineState = createInitialState(config);
  ...
  function dispatch(event: OverdubEngineEvent): Readonly<OverdubEngineState> {
    state.lastEventAt = nowIso();
    state.error = null;
    ...
    switch (event.type) {
      ...
      case 'SET_MONITORING_MODE': {
        const mode: MonitoringMode = event.monitoringMode;
  ...
  function getState(): Readonly<OverdubEngineState> {
    return state;
  }

  function reset(nextPartialConfig: Partial<OverdubEngineConfig> = {}): Readonly<OverdubEngineState> {
    config = {
      ...config,
      ...nextPartialConfig,
      defaultPhrase: {
