...
export type OverdubEngineEvent =
  | { type: 'NEW_PROJECT'; project?: Partial<OverdubProject> }
  | { type: 'ARM'; layerId?: string }
  | { type: 'START_COUNTIN' }
  | { type: 'START_REC' }
  | { type: 'CANCEL_REC' }
  | { type: 'STOP_REC_HARD'; take: OverdubTake }
  | { type: 'KEEP_TAKE' }
  | { type: 'REDO_TAKE' }
  | { type: 'UNDO_LAYER' }
  | { type: 'ADD_LAYER'; name?: string }
  | { type: 'REMOVE_LAYER'; layerId: string }
  | { type: 'SET_LAYER_MUTED'; layerId: string; muted: boolean }
  | { type: 'SET_LAYER_SOLO'; layerId: string; solo: boolean }
  | { type: 'SET_LAYER_GAIN'; layerId: string; gain: number }
  | { type: 'SET_LAYER_PAN'; layerId: string; pan: number }
  | { type: 'SET_ACTIVE_TAKE'; layerId: string; takeId: string }
  | { type: 'SET_PROJECT_TITLE'; title: string }
  | { type: 'SET_MONITORING_MODE'; monitoringMode: MonitoringMode }
  | { type: 'SET_CLICK_ENABLED'; clickEnabled: boolean }
  | { type: 'SET_PHRASE_SETTINGS'; phrase: Partial<PhraseSettings> }
  | { type: 'SET_PENDING_TAKE_OFFSET'; startOffsetMs: number }
  | { type: 'START_PLAYBACK' }
  | { type: 'STOP_PLAYBACK' }
  | { type: 'SET_PLAYBACK_TIME'; timeMs: number }
  | { type: 'START_EXPORT' }
  | { type: 'FINISH_EXPORT' }
