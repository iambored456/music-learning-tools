...
function normalizeBaseUnit(baseUnit) {
 ...
 */
export function getAdaptiveZoomStep(span, totalRanks, { maxStep = MAX_ADAPTIVE_ZOOM_STEP_ROWS } = {}) {
...
function rangeFromCenter(center, span, totalRanks) {
...
export function getSpan(range) {
    return (range.bottomIndex - range.topIndex) + 1;
}
export function clampToGamut(range, totalRanks) {
    const maxIndex = Math.max(0, totalRanks - 1);
 ...
 */
export function normalizeRange(range, totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS) {
...
export function getNormalizedRange(range, totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS) {
    const maxIndex = Math.max(0, totalRanks - 1);
 ...
 */
export function setTopEndpoint(current, requestedTop, totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS) {
    const maxIndex = Math.max(0, totalRanks - 1);
 ...
 */
export function setBottomEndpoint(current, requestedBottom, totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS) {
    const maxIndex = Math.max(0, totalRanks - 1);
 ...
 */
export function computeConstrainedTopIndex(currentBottomIndex, requestedTop, totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS) {
    const maxIndex = Math.max(0, totalRanks - 1);
 ...
 */
export function computeConstrainedBottomIndex(currentTopIndex, requestedBottom, totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS) {
    const maxIndex = Math.max(0, totalRanks - 1);
 ...
 */
export function shiftRangeBy(current, deltaRows, totalRanks) {
    const maxIndex = Math.max(0, totalRanks - 1);
 ...
 */
export function zoomRange(current, direction, { totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS, zoomStep = DEFAULT_ZOOM_STEP_ROWS }) {
    const maxIndex = Math.max(0, totalRanks - 1);
    const normalizedMinSpan = Math.max(1, Math.round(minSpan));
 ...
 */
export function buildSpanLadder(totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS) {
    const normalizedTotal = Math.max(1, Math.round(totalRanks));
    const normalizedMin = Math.max(1, Math.min(normalizedTotal, Math.round(minSpan)));
    const ladder = [normalizedTotal];
 ...
 */
export function snapSpanToLadder(span, ladder, mode = 'nearest') {
    if (!ladder.length) {
 ...
 */
export function snapRangeToSpanLadder(range, { totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS, ladder, mode = 'nearest' }) {
 ...
 */
export function zoomRangeOnSpanLadder(current, direction, { totalRanks, minSpan = DEFAULT_MIN_VIEWPORT_ROWS, ladder }) {
 ...
 */
export function calculateZoomToFitRowCount(containerHeight, rowCount, { baseUnit, paddingRows = 1 }) {
 ...
 */
export function calculateViewportBounds(pitchRange, totalRanks, cellHeight) {
    const maxIndex = Math.max(0, totalRanks - 1);
    const halfUnit = cellHeight / 2;
    const normalized = normalizeRange(pitchRange || { topIndex: 0, bottomIndex: maxIndex }, totalRanks, DEFAULT_MIN_VIEWPORT_ROWS);
 ...
 */
export function interpolateRange(startRange, targetRange, t, totalRanks) {
    const interpolatedTop = Math.round(startRange.topIndex + (targetRange.topIndex - startRange.topIndex) * t);
    const interpolatedBottom = Math.round(startRange.bottomIndex + (targetRange.bottomIndex - startRange.bottomIndex) * t);
    return normalizeRange({ topIndex: interpolatedTop, bottomIndex: interpolatedBottom }, totalRanks, DEFAULT_MIN_VIEWPORT_ROWS);
}
/**
 ...
 */
export function calculateViewportWindow(config) {
    const { containerHeight, fullRowData, preferredCellHeight = DEFAULT_PREFERRED_CELL_HEIGHT, minCellHeight = DEFAULT_MIN_CELL_HEIGHT, centerOnMidi, } = config;
 ...
 */
export function scrollViewportUp(currentWindow, totalRows, scrollRows = 1) {
 ...
 */
export function scrollViewportDown(currentWindow, totalRows, scrollRows = 1) {
