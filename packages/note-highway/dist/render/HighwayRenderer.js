...
import { createCoordinateMapper, } from '../coordinate/CoordinateMapper.js';
import { createGridRenderer } from './GridRenderer.js';
import { createNoteRenderer } from './NoteRenderer.js';
import { createJudgmentLineRenderer } from './JudgmentLineRenderer.js';
import { createInputCursorRenderer } from './InputCursorRenderer.js';
import { createInputTrailRenderer } from './InputTrailRenderer.js';
/**
 * Create a highway renderer instance.
 */
export function createHighwayRenderer(config = {}) {
    // Merge with defaults
    const fullConfig = mergeConfig(DEFAULT_HIGHWAY_CONFIG, config);
    ...
    function updateConfig(newConfig) {
        ...
        if (newConfig.viewport) {
            coordinateMapper.setViewportSize(fullConfig.viewport.width, fullConfig.viewport.height);
            coordinateMapper.viewport.setMidiRange(fullConfig.viewport.minMidi, fullConfig.viewport.maxMidi);
        }
    ...
    function render(ctx, state) {
        ...
        // 2. Draw grid
        gridRenderer.render(ctx, state.beats, state.currentTimeMs, coordinateMapper, viewport, fullConfig.gridRender);
        // 3. Draw tonic indicators (pitch-center highlights)
        if (fullConfig.tonicIndicator.enabled) {
            drawTonicIndicators(ctx, viewport);
        }
        // 4. Draw target notes
        const renderableNotes = coordinateMapper.notesToRenderables(state.notes, state.currentTimeMs, fullConfig.noteRender.noteHeight, state.activeNoteIds, state.passedNoteIds, state.judgments);
        noteRenderer.render(ctx, renderableNotes, fullConfig.noteRender);
        // 5. Draw input trail
        inputTrailRenderer.render(ctx, state.inputTrail, state.currentTimeMs, coordinateMapper, fullConfig.inputTrail);
        // 6. Draw input cursor
        inputCursorRenderer.render(ctx, state.inputCursor, coordinateMapper.getJudgmentLineX(), coordinateMapper, fullConfig.inputCursor);
        // 7. Draw judgment line (on top)
        judgmentLineRenderer.render(ctx, coordinateMapper.getJudgmentLineX(), viewport, state.inputCursor.isInTolerance, state.inputCursor.isVoiced, fullConfig.judgmentLine);
     ...
     */
    function drawTonicIndicators(ctx, viewport) {
     ...
     */
    function drawGateOverlay(ctx, viewport) {
