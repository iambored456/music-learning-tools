 ...
 */
export function getCentsOffset(midi: number): number {
 ...
 */
export function getNearestMidi(midi: number): number {
 ...
 */
export function midiToGridY(midi: number, config: PitchGridConfig): number {
 ...
 */
export function gridYToMidi(y: number, config: PitchGridConfig): number {
  const { topMidi, bottomMidi, gridHeight } = config;
  const totalSemitones = topMidi - bottomMidi + 1;
  const rowHeight = gridHeight / totalSemitones;
 ...
 */
export function getMidiRowCenterY(midi: number, config: PitchGridConfig): number {
  const { topMidi, bottomMidi, gridHeight } = config;
  const totalSemitones = topMidi - bottomMidi + 1;
  const rowHeight = gridHeight / totalSemitones;
 ...
 */
export function getRowHeight(config: PitchGridConfig): number {
  const { topMidi, bottomMidi, gridHeight } = config;
 ...
 */
export function isPitchAccurate(
  sungMidi: number,
  targetMidi: number,
 ...
 */
export function getPitchAccuracyScore(
  sungMidi: number,
  targetMidi: number,
  maxDeviationCents: number = CENTS_PER_SEMITONE
): number {
  const centsOff = Math.abs(sungMidi - targetMidi) * CENTS_PER_SEMITONE;
 ...
 */
export function getCentsDeviation(sungMidi: number, targetMidi: number): number {
 ...
 */
export function midiToNoteName(midi: number, preferFlats: boolean = false): string {
 ...
 */
export function processRawFrequency(
  frequency: number,
  minHz: number = 60,
  maxHz: number = 1600
): number | null {
