 ...
 */
export type SessionTimeMs = number & { readonly __brand: 'SessionTimeMs' };

/**
 * Audio context time in seconds.
 * Used when syncing with Web Audio API.
 */
export type AudioContextTime = number & { readonly __brand: 'AudioContextTime' };

/**
 * Scheduled event identifier.
 */
export type ScheduledEventId = string & { readonly __brand: 'ScheduledEventId' };
...
export function asSessionTimeMs(ms: number): SessionTimeMs {
  return ms as SessionTimeMs;
}

/** Create an AudioContextTime from a number */
export function asAudioContextTime(seconds: number): AudioContextTime {
  return seconds as AudioContextTime;
}

/** Create a ScheduledEventId from a string */
export function asScheduledEventId(id: string): ScheduledEventId {
  return id as ScheduledEventId;
}
...
export type SessionPhase =
  ...
  | 'loading'   // Chart being loaded
  | 'ready'     // Chart loaded, ready to start
  | 'playing'   // Session in progress
  | 'paused'    // Session paused by user
  | 'gated'     // Session paused by accuracy gate
