 ...
 */
export function createChartAdapter(config: Partial<ChartAdapterConfig> = {}): IChartAdapter {
  // Merge with defaults
  const fullConfig: ChartAdapterConfig = {
  ...
  let timeMap: SessionTimeMs[] = [];

  /**
   * Get the duration of one microbeat in milliseconds.
   */
  function getMicrobeatMs(): number {
   ...
   */
  function buildTimeMap(microbeatCount: number): void {
    timeMap = [];
   ...
   */
  function microbeatToMs(microbeatIndex: number): SessionTimeMs {
    if (timeMap.length === 0) {
   ...
   */
  function msToMicrobeatIndex(timeMs: SessionTimeMs): number {
    const microbeatMs = getMicrobeatMs();
   ...
   */
  function isShortNote(note: SnapshotNote): boolean {
   ...
   */
  function convertNote(note: SnapshotNote, voice: SnapshotVoice, noteIndex: number): TimedNote {
   ...
   */
  function buildBeats(timeGrid: TimeGridStructure): TimedBeat[] {
   ...
   */
  function loadSnapshot(snapshot: SingingTrainerSnapshot): ChartData {
   ...
   */
  function getChartData(): ChartData | null {
    return chartData;
  }
   ...
   */
  function getNotesInRange(startMs: SessionTimeMs, endMs: SessionTimeMs): TimedNote[] {
    if (!chartData) return [];
   ...
   */
  function getBeatsInRange(startMs: SessionTimeMs, endMs: SessionTimeMs): TimedBeat[] {
    if (!chartData) return [];

    return chartData.beats.filter(beat =>
      beat.timeMs >= startMs && beat.timeMs < endMs
    );
  }
   ...
   */
  function getNotesForVoice(voiceId: string): TimedNote[] {
    if (!chartData) return [];
