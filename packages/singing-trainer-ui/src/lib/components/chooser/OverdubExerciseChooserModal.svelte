<script lang="ts">
  ...
  const isVisible = $derived(overdubExerciseChooserState.state.isVisible);
  ...
  function handleClose() {
    overdubExerciseChooserState.hide();
    overdubExerciseChooserState.resetSelection();
    onclose?.();
  }

  function handleExerciseSelect(entry: RegistryEntry) {
    overdubExerciseChooserState.selectExercise(entry.template.id, entry.template.settingsSchema);
  }

  function handleSettingChange(key: string, value: number | boolean) {
    overdubExerciseChooserState.updateLocalSetting(key, value);
  }

  function handleStartExercise() {
    if (!selectedExerciseId) return;

    const settings = overdubExerciseChooserState.getLocalSettings();
    overdubExerciseChooserState.hide();
