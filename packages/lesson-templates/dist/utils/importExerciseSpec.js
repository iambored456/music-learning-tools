 ...
 */
export function convertSpecToExercise(spec, metadata) {
    ...
    const microbeatsPerMacrobeat = 2;
    ...
    const macrobeatGroupings = Array(totalMacrobeats).fill(2);
    const timeGrid = {
        microbeatCount,
    ...
    const voices = parts.map((part, i) => {
        ...
        for (const event of part.events) {
            ...
            if (typeof event.midi !== 'number')
                continue;
            if (event.midi < minMidi)
                minMidi = event.midi;
            if (event.midi > maxMidi)
                maxMidi = event.midi;
            notes.push({
