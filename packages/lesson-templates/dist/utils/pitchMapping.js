 ...
 */
export function resolveConfig(template, context) {
    ...
    if (speakingPitchMidi !== null && speakingPitchUsage !== 'none') {
        const mapped = applyPitchMapping(speakingPitchMidi, speakingPitchUsage, customPitchOffset, config);
 ...
 */
function applyPitchMapping(speakingPitch, usage, customOffset, config) {
    switch (usage) {
        case 'asTonic':
 ...
 */
export function applyAsTonic(speakingPitch, config) {
    // Calculate the desired range from config or use default
    const configRange = config.maxMidi !== undefined && config.minMidi !== undefined
 ...
 */
export function applyAsFloorNote(speakingPitch, config) {
    // Calculate the desired range from config or use default
    const configRange = config.maxMidi !== undefined && config.minMidi !== undefined
        ? config.maxMidi - config.minMidi
        : DEFAULT_RANGE_SEMITONES;
 ...
 */
export function applyCustomOffset(speakingPitch, offset, config) {
 ...
 */
export function applyVariation(baseConfig, overrides) {
    return {
        ...baseConfig,
        ...overrides,
    };
