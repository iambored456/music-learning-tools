 ...
 */
import { importMusicXML, } from '@mlt/musicxml-import';
import { convertSpecToExercise, } from './importExerciseSpec.js';
const DEGREE_BASE_PC = {
    1: 0,
    2: 2,
    3: 4,
    4: 5,
...
export function convertRelativeExerciseToSpec(exercise, options = {}) {
    const warnings = [];
    ...
    let maxDivEnd = 0;
    const parts = exercise.streams.map((stream) => {
        const baseName = (stream.name ?? stream.streamId).trim() || stream.streamId;
        ...
        stream.events.forEach((event, eventIndex) => {
            ...
            const d = Math.max(1, tickToDivision(event.dur, ticksPerDivision, warnings, {
...
export function importMusicXmlToExerciseSpec(xml, options = {}) {
...
export function importMusicXmlToOverdubExercise(xml, metadata, options = {}) {
...
function relativeEventToMidi(event, tonicMidi) {
    return tonicMidi + event.oct * 12 + DEGREE_BASE_PC[event.deg] + event.alt;
}
function tickToDivision(ticks, ticksPerDivision, warnings, context) {
    ...
    if (Math.abs(raw - rounded) > 1e-9) {
        warnings.push({
            ...
            message: context.kind === 'POSITION_ROUNDED'
...
function fifthsToTonicPc(fifths) {
