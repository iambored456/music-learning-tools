...
import type { LessonTemplate, PitchMatchingTemplate, ExercisePattern, LoopPhase } from '../types.js';
 ...
 */
export function validateTemplate(template: LessonTemplate): ValidationResult {
  const errors: string[] = [];
  ...
  if (template.type === 'pitch-matching') {
    const pitchMatchingErrors = validatePitchMatchingTemplate(template as PitchMatchingTemplate);
 ...
 */
function validatePitchMatchingTemplate(template: PitchMatchingTemplate): string[] {
  const errors: string[] = [];
 ...
 */
function validatePattern(pattern: ExercisePattern): string[] {
  const errors: string[] = [];
  ...
  for (let i = 0; i < pattern.phases.length; i++) {
    ...
    if (phase.type === 'reference') hasReference = true;
    if (phase.type === 'input') hasInput = true;
  }
 ...
 */
function validatePhase(phase: LoopPhase, index: number): string[] {
  const errors: string[] = [];
 ...
 */
export function calculateLoopDurationMicrobeats(pattern: ExercisePattern): number {
 ...
 */
export function calculateLoopDurationMs(pattern: ExercisePattern, tempo: number): number {
