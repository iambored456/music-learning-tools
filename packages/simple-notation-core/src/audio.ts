...
export type ScheduledPlaybackAudioRun = {
...
export class SimpleNotationAudioEngine {
  ...
  private scheduledPlaybackStartedAtPerformanceMs: number | null = null;
  ...
  schedulePlaybackAudio(events: ScheduledPlaybackAudioEvent[], startDelaySeconds = 0.03): ScheduledPlaybackAudioRun | null {
    ...
    const resolvedStartDelaySeconds = Math.max(0, startDelaySeconds);
    const scheduledAtPerformanceMs = typeof performance !== 'undefined' ? performance.now() : 0;
    this.scheduledPlaybackStartedAtPerformanceMs = scheduledAtPerformanceMs + resolvedStartDelaySeconds * 1000;
    Tone.Transport.start(Tone.now() + resolvedStartDelaySeconds);
    return {
      startDelaySeconds: resolvedStartDelaySeconds,
      startedAtPerformanceMs: this.scheduledPlaybackStartedAtPerformanceMs,
    };
  }

  getScheduledPlaybackElapsedMs(): number | null {
    if (this.scheduledPlaybackStartedAtPerformanceMs === null || typeof performance === 'undefined') {
      return null;
    }

    return performance.now() - this.scheduledPlaybackStartedAtPerformanceMs;
  }
  ...
  stopScheduledPlaybackAudio(): void {
    ...
    Tone.Transport.stop();
    Tone.Transport.position = 0;
    this.scheduledPlaybackStartedAtPerformanceMs = null;
