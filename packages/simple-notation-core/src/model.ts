...
export type SimpleNotationListener = (state: SimpleNotationState) => void;
...
export class SimpleNotationModel {
  private state: SimpleNotationState;

  private listeners = new Set<SimpleNotationListener>();

  constructor(initial?: Partial<SimpleNotationState>) {
    this.state = {
      ...createInitialState(),
      ...(initial ?? {}),
    };
  ...
  subscribe(listener: SimpleNotationListener): () => void {
  ...
  setKeyboardFocusZone(zone: SimpleNotationState['keyboardFocusZone'], notify = true): void {
  ...
  private updateState(patch: Partial<SimpleNotationState>, options: UpdateOptions = {}): void {
...
export function createSimpleNotationModel(initial?: Partial<SimpleNotationState>): SimpleNotationModel {
  return new SimpleNotationModel(initial);
}

export function createDefaultSimpleNotationModel(): SimpleNotationModel {
  return new SimpleNotationModel();
}
