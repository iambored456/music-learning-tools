<script lang="ts">
  ...
  import {
    CHROMANOTES_PALETTE as SIMPLE_NOTATION_CHROMANOTES_PALETTE,
    COLOR_PALETTE,
    DEFAULTS,
    KEY_CODES,
    MICROBEAT_TEMPO_MAX,
  ...
  import {
    deleteSimpleNotationLibraryEntry,
    listSimpleNotationLibraryEntries,
    saveSimpleNotationLibraryEntry,
    type SimpleNotationLibraryEntry,
  ...
  type ContinuousKaraokeAnchorTransition = KaraokeAnchorTransition & {
    sourceIndex: number;
    sourceAnchor: KaraokeAnchor;
  };
  ...
  type PlaybackTimeline = {
    ...
    leadInMs: number;
    countIn: boolean;
    countInStartupDelayMs: number;
    transportStartAtMs: number;
    musicStartAtMs: number;
  };
  ...
  type LibrarySketchExportDocument = {
    v: 1;
    source: 'simple-notation-library';
    name: string;
    savedAt: string;
    exportedAt: string;
  ...
  const CHROMANOTES_PALETTE: Record<string, string> = {
    ...SIMPLE_NOTATION_CHROMANOTES_PALETTE,
  ...
  const PLAYBACK_PREFERENCES_KEY = 'simple-notation-ui:playback-preferences:v1';
  const PLAYBACK_PREFERENCES_VERSION = 1;
  const DISPLAY_PREFERENCES_KEY = 'simple-notation-ui:display-preferences:v1';
  ...
  const HORIZONTAL_PLAYBACK_DEBUG_FRAME_INTERVAL_MS = 1000;
  const HORIZONTAL_PLAYBACK_DEBUG_PROBLEM_FRAME_INTERVAL_MS = 250;
  const HORIZONTAL_PLAYBACK_DEBUG_VERBOSE_FRAME_INTERVAL_MS = 125;
  ...
  const PLACED_SIXTEENTH_HEX_PATH = createSixteenthHexPath(12.5, 50, 25, 96);
  ...
  let activePlaybackTimeline: PlaybackTimeline | null = null;
  ...
  let horizontalPlaybackDebugFrameCount = 0;
  let horizontalPlaybackDebugLastMotionIndex: number | null = null;
  let horizontalPlaybackDebugLastMotionLogAtMs = 0;
  let horizontalScrollDebugEveryFrame = false;
  ...
  let libraryFileName = '';
  let libraryEntries: Array<SimpleNotationLibraryEntry<LibrarySketchDocument>> = [];
  let librarySelectedEntryId: string | null = null;
  let selectedLibraryEntryForActions: SimpleNotationLibraryEntry<LibrarySketchDocument> | null = null;
  ...
  function refreshHorizontalPlaybackSizing(): void {
  ...
  $: {
    ...
    refreshHorizontalPlaybackSizing();
  }
  $: {
    voiceRows;
    voicePickupRows;
  ...
  onMount(() => {
    ...
    return () => {
      stopPlayback('component destroyed');
  ...
  function setPickupBeats(nextBeats: number): void {
    ...
    if (isPlaying) {
      stopPlayback('pickup beats changed');
    }
  }
  ...
  function buildLibrarySketchExportDocument(
    entry: SimpleNotationLibraryEntry<LibrarySketchDocument>,
  ...
  function downloadLibraryEntry(entry: SimpleNotationLibraryEntry<LibrarySketchDocument>): void {
  ...
  function selectedLibraryEntry(): SimpleNotationLibraryEntry<LibrarySketchDocument> | null {
  ...
  async function refreshLibraryEntries(preferredEntryId: string | null = librarySelectedEntryId): Promise<void> {
    const storedEntries = await listSimpleNotationLibraryEntries<LibrarySketchDocument>();
  ...
  async function handleLibrarySaveAs(): Promise<void> {
    ...
    try {
      const entry = await saveSimpleNotationLibraryEntry<LibrarySketchDocument>({
        name: nextName,
  ...
  async function confirmLibraryAction(): Promise<void> {
    ...
    try {
      ...
      await deleteSimpleNotationLibraryEntry(entry.id);
  ...
  function loadFromShareDocument(
    ...
    if (isPlaying) {
      stopPlayback('share document loaded');
    }
  ...
  function waitForAnimationFrame(): Promise<number> {
    if (typeof requestAnimationFrame === 'undefined') {
      return Promise.resolve(typeof performance !== 'undefined' ? performance.now() : 0);
    }
  ...
  async function settlePlaybackLayout(startToken: number): Promise<boolean> {
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Settling horizontal playback layout.', horizontalScrollDiagnosticsSnapshot({
        startToken,
      }));
    }

    await tick();
    if (!isPlaying || playbackStartToken !== startToken) return false;
    ...
    if (trackStyle === 'horizontal') {
      for (let frameIndex = 0; frameIndex < HORIZONTAL_PLAYBACK_LAYOUT_SETTLE_FRAMES; frameIndex += 1) {
    ...
    applyKaraokeBallElementStyles();
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Horizontal playback layout settled.', horizontalScrollDiagnosticsSnapshot({
        startToken,
      }));
    }
    return true;
  ...
  function handleVoiceCountModeChange(event: Event): void {
    ...
    if (isPlaying) {
      stopPlayback('voice count changed');
    }
  }
  ...
  function handleVoiceLayoutModeChange(event: Event): void {
    ...
    voiceLayoutMode = nextVoiceLayoutMode;
    if (isPlaying) {
      stopPlayback('voice layout changed');
    }
  }
  ...
  function removeRow(): void {
    ...
    if (playbackIndex >= totalCells) {
      stopPlayback('row removed past playback index');
    }
  }
  ...
  function debugTempoSliderLayout(message: string, details?: Record<string, unknown>): void {
    if (!TEMPO_SLIDER_LAYOUT_DEBUG) return;
    console.log(`[SimpleNotation][TempoSlider] ${message}`, details ?? {});
  }

  function debugPlaybackStartup(message: string, details?: Record<string, unknown>): void {
    if (!PLAYBACK_STARTUP_DEBUG) return;
    console.info(`[SimpleNotation][PlaybackStartup] ${message}`, details ?? {});
  }
  ...
  function isHorizontalScrollEveryFrameValue(value: string | null): boolean {
    return ['all', 'frames'].includes(normalizedDebugFlag(value));
  }
  ...
  function refreshHorizontalScrollDebugFlags(): void {
    ...
    horizontalScrollDebugEveryFrame = isHorizontalScrollEveryFrameValue(verboseValue);
    ...
    if (horizontalScrollDebugEnabled && !wasEnabled) {
      console.info('[SimpleNotation][HorizontalScroll] Diagnostics enabled.', {
        verbose: horizontalScrollDebugVerbose,
        everyFrame: horizontalScrollDebugEveryFrame,
        ...
        everyFrameCommand: `localStorage.setItem("${HORIZONTAL_SCROLL_DEBUG_STORAGE_KEY}", "frames")`,
  ...
  function debugHorizontalScroll(message: string, details?: Record<string, unknown>): void {
    ...
    console.info(`[SimpleNotation][HorizontalScroll] ${message}`, details ?? {});
  }
  ...
  function karaokeAnchorDebugSnapshot(anchor: KaraokeAnchor | null): Record<string, unknown> | null {
    return anchor
      ? {
          rowIndex: anchor.rowIndex,
          leftPercent: roundedMetric(anchor.leftPercent),
        }
      : null;
  }

  function playbackTimelineDebugSnapshot(
    timeline: PlaybackTimeline | null = activePlaybackTimeline,
    extra: Record<string, unknown> = {},
  ): Record<string, unknown> {
    const nowMs = typeof performance !== 'undefined' ? performance.now() : 0;
    const scheduledElapsedMs = audio.getScheduledPlaybackElapsedMs();
    const musicElapsedMs = timeline
      ? scheduledElapsedMs !== null
        ? scheduledElapsedMs - timeline.leadInMs
        : nowMs - timeline.musicStartAtMs
      : null;

    return {
      ...extra,
      nowMs: roundedMetric(nowMs),
      playbackIndex,
      playbackVisualStartIndex,
      playbackVisualLastRenderedIndex,
      playbackVisualStartedAtMs: roundedMetric(playbackVisualStartedAtMs),
      audioScheduleStartDelayMs: AUDIO_SCHEDULE_START_DELAY_MS,
      scheduledElapsedMs: roundedMetric(scheduledElapsedMs),
      musicElapsedMs: roundedMetric(musicElapsedMs),
      timeUntilTransportStartMs: timeline ? roundedMetric(timeline.transportStartAtMs - nowMs) : null,
      timeUntilMusicStartMs: timeline ? roundedMetric(timeline.musicStartAtMs - nowMs) : null,
      countInDisplayNumber,
      countInEnabled,
      activeTimeline: timeline
        ? {
            startIndex: timeline.startIndex,
            traversalEndIndex: timeline.traversalEndIndex,
            intervalMs: timeline.intervalMs,
            leadInMs: timeline.leadInMs,
            countIn: timeline.countIn,
            countInStartupDelayMs: timeline.countInStartupDelayMs,
            transportStartAtMs: roundedMetric(timeline.transportStartAtMs),
            musicStartAtMs: roundedMetric(timeline.musicStartAtMs),
          }
        : null,
    };
  }

  function horizontalPlaybackAnchorDebugSnapshot(
    index: number,
    traversalEndIndex: number,
    startIndex: number = activePlaybackTimeline?.startIndex ?? playbackVisualStartIndex,
  ): Array<Record<string, unknown>> {
    const intervalMs = activePlaybackTimeline?.intervalMs ?? playbackIntervalMs();

    return visibleVoiceIndices().map((voiceIndex) => {
      const currentAnchor = karaokeAnchorFromPlaybackIndex(voiceIndex, index);
      const transition = currentAnchor
        ? continuousKaraokeAnchorTransition(voiceIndex, index, startIndex, traversalEndIndex, currentAnchor, intervalMs)
        : null;
      const highlightAnchor = voicePlaybackHighlightAnchors[voiceIndex];
      const currentPanelLeftPx = currentAnchor ? karaokeAnchorViewportLeftPx(voiceIndex, currentAnchor, 'panel') : null;
      const currentContentLeftPx = currentAnchor ? karaokeAnchorContentLeftPx(voiceIndex, currentAnchor) : null;
      const targetPanelLeftPx = transition ? karaokeAnchorViewportLeftPx(voiceIndex, transition.anchor, 'panel') : null;
      const targetContentLeftPx = transition ? karaokeAnchorContentLeftPx(voiceIndex, transition.anchor) : null;
      const rowElement = currentAnchor ? getTrackRowElement(voiceIndex, currentAnchor.rowIndex) : null;

      return {
        voiceIndex,
        visible: isVoiceVisible(voiceIndex),
        currentAnchor: karaokeAnchorDebugSnapshot(currentAnchor),
        transition: transition
          ? {
              sourceIndex: transition.sourceIndex,
              sourceAnchor: karaokeAnchorDebugSnapshot(transition.sourceAnchor),
              targetIndex: transition.targetIndex,
              durationMs: transition.durationMs,
              anchor: karaokeAnchorDebugSnapshot(transition.anchor),
            }
          : null,
        currentPanelLeftPx: roundedMetric(currentPanelLeftPx),
        targetPanelLeftPx: roundedMetric(targetPanelLeftPx),
        currentContentLeftPx: roundedMetric(currentContentLeftPx),
        targetContentLeftPx: roundedMetric(targetContentLeftPx),
        pinnedLeftPx: roundedMetric(voiceKaraokeBallPinnedLeftPxs[voiceIndex]),
        laneShiftPx: roundedMetric(voiceHorizontalPlaybackLaneShiftPxs[voiceIndex]),
        rowTransform: rowElement?.style.transform || null,
        renderedBall: {
          rowIndex: voiceKaraokeBallRowIndexes[voiceIndex],
          leftPercent: roundedMetric(voiceKaraokeBallLeftPercents[voiceIndex]),
          arcOffsetPx: roundedMetric(voiceKaraokeBallArcOffsetPxs[voiceIndex]),
        },
        highlightAnchor: karaokeAnchorDebugSnapshot(highlightAnchor),
        highlightMatchesCurrent:
          !!currentAnchor && !!highlightAnchor && karaokeAnchorsEqual(currentAnchor, highlightAnchor),
      };
    });
  }

  function horizontalPlaybackLaneShiftDebugSnapshot(): Array<Record<string, unknown>> {
    return visibleVoiceIndices().map((voiceIndex) => {
      const rowIndex = voiceKaraokeBallRowIndexes[voiceIndex] ?? 0;
      const rowElement = getTrackRowElement(voiceIndex, rowIndex);
      ...
      return {
        ...
        laneShiftPx: roundedMetric(voiceHorizontalPlaybackLaneShiftPxs[voiceIndex]),
        pinnedLeftPx: roundedMetric(voiceKaraokeBallPinnedLeftPxs[voiceIndex]),
        ballRowIndex: voiceKaraokeBallRowIndexes[voiceIndex],
        ballLeftPercent: roundedMetric(voiceKaraokeBallLeftPercents[voiceIndex]),
        ballArcOffsetPx: roundedMetric(voiceKaraokeBallArcOffsetPxs[voiceIndex]),
        rowTransform: rowElement?.style.transform || null,
        rowWillChange: rowElement?.style.willChange || null,
      };
    });
  }
  ...
  function horizontalScrollDiagnosticsSnapshot(extra: Record<string, unknown> = {}): Record<string, unknown> {
    ...
    return {
      ...
      laneShifts: horizontalPlaybackLaneShiftDebugSnapshot(),
      playbackTimeline: playbackTimelineDebugSnapshot(),
  ...
  function recordHorizontalScrollDiagnosticsFrame(
    ...
    const shouldLogFrame = horizontalScrollDebugEveryFrame || slowFrame || longApply || positionError || transformMismatch;
  ...
  function queuePlaybackScrollForCurrentStep(currentIndex: number, totalCells: number, stepStartedAtMs: number): void {
    ...
    if (activePlaybackTimeline) {
      if (horizontalScrollDebugEnabled) {
        debugHorizontalScroll('Skipping discrete playback scroll: continuous timeline motion is active.', {
          currentIndex,
          totalCells,
        });
      }
  ...
  function nextDistinctKaraokeAnchorTransition(
    ...
    intervalMs: number = playbackIntervalMs(),
    ...
    for (let targetIndex = currentIndex + 1; targetIndex < totalCells; targetIndex += 1) {
      ...
      return {
        anchor: targetAnchor,
        targetIndex,
        durationMs: Math.max(24, Math.floor((targetIndex - currentIndex) * intervalMs)),
      };
    }
  ...
  function karaokeAnchorRunStartIndex(
    voiceIndex: VoiceIndex,
    currentIndex: number,
    startIndex: number,
    ...
    for (let previousIndex = currentIndex - 1; previousIndex >= startIndex; previousIndex -= 1) {
      const previousAnchor = karaokeAnchorFromPlaybackIndex(voiceIndex, previousIndex);
      if (!previousAnchor || !karaokeAnchorsEqual(previousAnchor, currentAnchor)) break;
  ...
  function continuousKaraokeAnchorTransition(
    voiceIndex: VoiceIndex,
    currentIndex: number,
    startIndex: number,
    totalCells: number,
    currentAnchor: KaraokeAnchor,
    intervalMs: number = playbackIntervalMs(),
  ): ContinuousKaraokeAnchorTransition | null {
    const sourceIndex = karaokeAnchorRunStartIndex(voiceIndex, currentIndex, startIndex, currentAnchor);

    for (let targetIndex = currentIndex + 1; targetIndex < totalCells; targetIndex += 1) {
      const targetAnchor = karaokeAnchorFromPlaybackIndex(voiceIndex, targetIndex);
      if (!targetAnchor || karaokeAnchorsEqual(currentAnchor, targetAnchor)) continue;

      return {
        sourceIndex,
        sourceAnchor: currentAnchor,
        anchor: targetAnchor,
        targetIndex,
        durationMs: Math.max(24, Math.floor((targetIndex - sourceIndex) * intervalMs)),
      };
    }
  ...
  function updateContinuousHorizontalPlaybackMotion(elapsedMs: number, intervalMs: number): void {
    const nowMs = typeof performance !== 'undefined' ? performance.now() : 0;
    const referenceViewportLeftPx = horizontalPlaybackHighway.referenceViewportLeftPx;
    if (trackStyle !== 'horizontal') return;
    if (referenceViewportLeftPx === null) {
      if (
        horizontalScrollDebugEnabled &&
        (horizontalScrollDebugVerbose ||
          nowMs - horizontalPlaybackDebugLastMotionLogAtMs >= HORIZONTAL_PLAYBACK_DEBUG_PROBLEM_FRAME_INTERVAL_MS)
      ) {
        horizontalPlaybackDebugLastMotionLogAtMs = nowMs;
        debugHorizontalScrollFrame('Continuous horizontal playback skipped: highway reference is missing.', horizontalScrollDiagnosticsSnapshot({
          elapsedMs: roundedMetric(elapsedMs),
          intervalMs,
        }));
      }
    ...
    const totalCells = totalPlaybackCells();
    if (totalCells <= 0) {
      debugHorizontalScroll('Continuous horizontal playback skipped: no playback cells.', {
        elapsedMs: roundedMetric(elapsedMs),
        intervalMs,
      });
      return;
    ...
    const startIndex = timeline?.startIndex ?? playbackVisualStartIndex;
    const traversalEndIndex = timeline?.traversalEndIndex
      ?? (isHorizontalLoopPlaybackActive() ? horizontalLoopDisplayEndIndex(totalCells) : totalCells);
    if (traversalEndIndex <= startIndex) {
      debugHorizontalScroll('Continuous horizontal playback skipped: invalid traversal bounds.', playbackTimelineDebugSnapshot(timeline, {
        startIndex,
        traversalEndIndex,
        totalCells,
      }));
    ...
    const playbackPosition = startIndex + Math.max(0, elapsedMs) / intervalMs;
    const currentIndex = Math.max(startIndex, Math.min(traversalEndIndex - 1, Math.floor(playbackPosition)));

    let preferredScrollTarget: number | null = null;
    let missingAnchorCount = 0;
    let geometryMissCount = 0;
    for (const voiceIndex of visibleVoiceIndices()) {
      ...
      const transition = continuousKaraokeAnchorTransition(
        voiceIndex,
        currentIndex,
        startIndex,
        traversalEndIndex,
      ...
      const transitionElapsedMs = transition
        ? Math.max(0, elapsedMs - (transition.sourceIndex - startIndex) * intervalMs)
        : 0;
      const progress = transition ? clamp(transitionElapsedMs / transition.durationMs, 0, 1) : 0;
      const displayAnchor = transition && progress >= 1 ? transition.anchor : currentAnchor;
      const arcOffsetPx = transition ? -4 * karaokeArcHeightPx * progress * (1 - progress) : 0;
      setKaraokeBallState(voiceIndex, displayAnchor.rowIndex, displayAnchor.leftPercent, arcOffsetPx);
      voiceKaraokeAnchors[voiceIndex] = displayAnchor;

      const currentAnchorPanelLeftPx = karaokeAnchorViewportLeftPx(voiceIndex, currentAnchor, 'panel');
      const targetAnchorPanelLeftPx =
        transition ? karaokeAnchorViewportLeftPx(voiceIndex, transition.anchor, 'panel') : currentAnchorPanelLeftPx;
      if (currentAnchorPanelLeftPx === null || targetAnchorPanelLeftPx === null) {
        geometryMissCount += 1;
        continue;
      }

      const interpolatedPanelLeftPx =
        currentAnchorPanelLeftPx + (targetAnchorPanelLeftPx - currentAnchorPanelLeftPx) * progress;
      const pinnedLeftPx = voiceKaraokeBallPinnedLeftPxs[voiceIndex];
      if (voiceCount > 1 && pinnedLeftPx !== null) {
        setHorizontalPlaybackLaneShiftPx(voiceIndex, pinnedLeftPx - interpolatedPanelLeftPx);
        continue;
      }

      const preferredVoiceIndex = horizontalPlaybackHighway.pinnedVoiceIndex ?? resolvePlaybackScrollVoiceIndex();
      if (voiceIndex !== preferredVoiceIndex) continue;

      const currentContentLeftPx = karaokeAnchorContentLeftPx(voiceIndex, currentAnchor);
      const targetContentLeftPx = transition ? karaokeAnchorContentLeftPx(voiceIndex, transition.anchor) : currentContentLeftPx;
      if (currentContentLeftPx === null || targetContentLeftPx === null) {
        geometryMissCount += 1;
        continue;
      }

      const interpolatedContentLeftPx =
        currentContentLeftPx + (targetContentLeftPx - currentContentLeftPx) * progress;
      preferredScrollTarget = interpolatedContentLeftPx - referenceViewportLeftPx;
    }

    if (voiceCount <= 1 && preferredScrollTarget !== null) {
      setHorizontalPlaybackScrollLeft(preferredScrollTarget);
    }

    const missingSingleVoiceScrollTarget = voiceCount <= 1 && preferredScrollTarget === null;
    const hasProblem = missingAnchorCount > 0 || geometryMissCount > 0 || missingSingleVoiceScrollTarget;
    const frameLogIntervalMs = hasProblem
      ? HORIZONTAL_PLAYBACK_DEBUG_PROBLEM_FRAME_INTERVAL_MS
      : HORIZONTAL_PLAYBACK_DEBUG_FRAME_INTERVAL_MS;
    const shouldLogFrame =
      horizontalScrollDebugEnabled &&
      (
        horizontalScrollDebugEveryFrame ||
        currentIndex !== horizontalPlaybackDebugLastMotionIndex ||
        nowMs - horizontalPlaybackDebugLastMotionLogAtMs >= (
          horizontalScrollDebugVerbose
            ? Math.min(frameLogIntervalMs, HORIZONTAL_PLAYBACK_DEBUG_VERBOSE_FRAME_INTERVAL_MS)
            : frameLogIntervalMs
        )
      );

    if (shouldLogFrame) {
      horizontalPlaybackDebugFrameCount += 1;
      horizontalPlaybackDebugLastMotionIndex = currentIndex;
      horizontalPlaybackDebugLastMotionLogAtMs = nowMs;
      debugHorizontalScrollFrame('Continuous horizontal playback motion frame.', horizontalScrollDiagnosticsSnapshot({
        horizontalPlaybackDebugFrameCount,
        elapsedMs: roundedMetric(elapsedMs),
        intervalMs,
        playbackPosition: roundedMetric(playbackPosition),
        currentIndex,
        startIndex,
        traversalEndIndex,
        referenceViewportLeftPx: roundedMetric(referenceViewportLeftPx),
        preferredScrollTarget: roundedMetric(preferredScrollTarget),
        missingAnchorCount,
        geometryMissCount,
        missingSingleVoiceScrollTarget,
        anchors: horizontalPlaybackAnchorDebugSnapshot(currentIndex, traversalEndIndex, startIndex),
      }));
    }
  }
  ...
  function startKaraokeLeadIn(
    voiceIndex: VoiceIndex,
    firstAnchor: KaraokeAnchor,
    startedAtMs: number = performance.now(),
  ): number {
  ...
  function queuePlaybackTimeoutAt(callback: () => void, targetAtMs: number): void {
    queuePlaybackTimeout(callback, Math.max(0, targetAtMs - performance.now()));
  }
  ...
  function schedulePlaybackAudioRun(
    ...
    const scheduledRun = audio.schedulePlaybackAudio(events, AUDIO_SCHEDULE_START_DELAY_MS / 1000);
    if (!scheduledRun) {
      debugHorizontalScroll('Playback audio scheduling failed.', {
        startIndex,
        traversalEndIndex,
        timelineOffsetMs,
        includeCountIn,
        countInStartupDelayMs,
    ...
    const timeline: PlaybackTimeline = {
      ...
      leadInMs: timelineOffsetMs,
      countIn: includeCountIn,
      countInStartupDelayMs,
      transportStartAtMs: scheduledRun.startedAtPerformanceMs,
      musicStartAtMs: scheduledRun.startedAtPerformanceMs + timelineOffsetMs,
    };
    debugHorizontalScroll('Playback audio run scheduled.', playbackTimelineDebugSnapshot(timeline, {
      eventCount: events.length,
      startDelaySeconds: scheduledRun.startDelaySeconds,
    }));
    return timeline;
  }

  function playbackMusicElapsedMs(now: number): number {
    const timeline = activePlaybackTimeline;
    if (!timeline) return now - playbackVisualStartedAtMs;

    const scheduledElapsedMs = audio.getScheduledPlaybackElapsedMs();
    if (scheduledElapsedMs !== null) {
      return scheduledElapsedMs - timeline.leadInMs;
    }

    return now - timeline.musicStartAtMs;
  }

  function startPlaybackVisualLoop(startIndex: number, musicStartAtMs = performance.now()): void {
    clearPlaybackTimer();
    const loopToken = playbackVisualLoopToken;
    playbackVisualStartedAtMs = musicStartAtMs;
    ...
    horizontalPlaybackDebugFrameCount = 0;
    horizontalPlaybackDebugLastMotionIndex = null;
    horizontalPlaybackDebugLastMotionLogAtMs = 0;
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Playback visual loop started.', playbackTimelineDebugSnapshot(activePlaybackTimeline, {
        startIndex,
        loopToken,
      }));
    }
    ...
    const step = (now: number): void => {
      ...
      const intervalMs = activePlaybackTimeline?.intervalMs ?? playbackIntervalMs();
      const elapsedMs = playbackMusicElapsedMs(now);
      if (elapsedMs < 0) {
        playbackFrame = requestAnimationFrame(step);
        return;
      }

      const elapsedSteps = Math.max(0, Math.floor(elapsedMs / intervalMs));
      ...
      while (isPlaying && loopToken === playbackVisualLoopToken && playbackVisualLastRenderedIndex < targetIndex && renderedSteps < 4) {
        const nextRenderedIndex = playbackVisualLastRenderedIndex + 1;
        const scheduledStepAtMs = playbackVisualStartedAtMs + (nextRenderedIndex - playbackVisualStartIndex) * intervalMs;
        playbackStep(scheduledStepAtMs);
        playbackVisualLastRenderedIndex += 1;
        renderedSteps += 1;
      }

      updateContinuousHorizontalPlaybackMotion(elapsedMs, intervalMs);
      ...
      if (horizontalScrollDebugEnabled && trackStyle === 'horizontal') {
        ...
        if (horizontalScrollDebugEveryFrame || slowFrame || catchUpFrame || longWork) {
  ...
  function selectPlaybackStartBeginning(): void {
    if (pickupMicrobeatCount() > 0) {
      playbackStartSelection = { zone: 'pickup', rowIndex: -1, startIndex: 0 };
    } else {
      const startIndex = playbackStartIndexForMeasure('main', 0);
      playbackStartSelection = startIndex === null ? null : { zone: 'main', rowIndex: 0, startIndex };
    }
  ...
  function handleMeasureLabelPointerDown(event: PointerEvent, voiceIndex: VoiceIndex): void {
    event.stopPropagation();
    if (!isPlaying) setActiveCanvasVoice(voiceIndex);
  }

  function handleMeasureLabelClick(event: MouseEvent, voiceIndex: VoiceIndex, sourceRowIndex: number): void {
    event.preventDefault();
    event.stopPropagation();
    if (isPlaying) return;

    setActiveCanvasVoice(voiceIndex);
    selectPlaybackStartMeasure('main', sourceRowIndex);
  }
  ...
  function isPlaybackStartLabelSelected(sourceRowIndex: number): boolean {
    if (!playbackStartSelection) return false;
    if (playbackStartSelection.zone === 'pickup') return sourceRowIndex === 0;
    return playbackStartSelection.rowIndex === sourceRowIndex;
  }
  ...
  function playbackStep(stepStartedAtMs = performance.now()): void {
    const totalCells = totalPlaybackCells();
    if (totalCells <= 0) {
      debugPlaybackHighlight('Stopping playback: no cells available.');
      stopPlayback('no playback cells during step');
      return;
    }
    ...
    if (trackStyle === 'horizontal' && horizontalScrollDebugEnabled) {
      debugHorizontalScroll('Horizontal playback step.', horizontalScrollDiagnosticsSnapshot({
        currentIndex,
        traversalEndIndex,
        cursor,
        stepStartedAtMs: roundedMetric(stepStartedAtMs),
        stepLagMs: roundedMetric(performance.now() - stepStartedAtMs),
        anchors: horizontalPlaybackAnchorDebugSnapshot(currentIndex, traversalEndIndex),
      }));
    }
    ...
    for (const voiceIndex of visibleVoiceIndices()) {
      ...
      updateKaraokeAfterPlaybackStep(voiceIndex, currentIndex, traversalEndIndex, cursor);
    ...
    if (playbackIndex >= totalCells) {
      clearPlaybackTimer();
      const finalCellDurationMs = activePlaybackTimeline?.intervalMs ?? playbackIntervalMs();
      ...
      if (isLooping) {
        queuePlaybackTimeout(() => {
          ...
          restartPlaybackTimer();
        }, Math.max(0, finalCellDurationMs - AUDIO_SCHEDULE_START_DELAY_MS));
      } else {
        queuePlaybackTimeout(() => {
          stopPlayback('playback complete');
        }, finalCellDurationMs);
      }
    }
  }
  ...
  function restartPlaybackTimer(): void {
    ...
    const timeline = schedulePlaybackAudioRun(currentIndex, traversalEndIndex);
    if (!timeline) {
      stopPlayback('restart audio scheduling failed');
      return;
    }

    activePlaybackTimeline = timeline;
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Horizontal playback timeline restarted.', playbackTimelineDebugSnapshot(timeline, {
        currentIndex,
        traversalEndIndex,
        anchors: horizontalPlaybackAnchorDebugSnapshot(currentIndex, traversalEndIndex),
      }));
    }
    startPlaybackVisualLoop(currentIndex, timeline.musicStartAtMs);
  }
  ...
  function triggerCountInBeat(
    ...
    scheduledAtMs: number = performance.now(),
  ): void {
    ...
    for (const [voiceIndex, anchor] of firstAnchors) {
      ...
      if (accented) {
        animateKaraokeBallVertical(voiceIndex, anchor, beatMs, karaokeArcHeightPx, scheduledAtMs);
      } else {
        clearKaraokeAnimation(voiceIndex);
        setKaraokeBallToAnchor(voiceIndex, anchor);
      }
  ...
  function startCountIn(
    ...
    const stableAnchors = new Map(currentAnchors);

    for (const [voiceIndex, anchor] of stableAnchors) {
      clearKaraokeAnimation(voiceIndex);
      setKaraokeBallToAnchor(voiceIndex, anchor);
      voiceKaraokeAnchors[voiceIndex] = anchor;
    }
    ...
    for (let index = 0; index < COUNT_IN_NUMBERS.length; index += 1) {
      ...
      const scheduledAtMs = transportStartAtMs + scheduledDelayMs;
      queuePlaybackTimeoutAt(() => {
        if (!isPlaying) return;
        triggerCountInBeat(
          stableAnchors,
          index,
          beatMs,
          scheduledDelayMs,
          Math.round(performance.now() - transportStartAtMs),
          scheduledAtMs,
        );
      }, scheduledAtMs);
    }
  ...
  function karaokeLeadInDurationMs(): number {
    return Math.max(40, Math.floor(playbackIntervalMs() / 2));
  }

  function startKaraokeLeadInAt(firstAnchors: Map<VoiceIndex, KaraokeAnchor>, startsAtMs: number): number {
    const leadInMs = karaokeLeadInDurationMs();
    for (const [voiceIndex, anchor] of firstAnchors) {
      clearKaraokeAnimation(voiceIndex);
      setKaraokeBallToAnchor(voiceIndex, anchor);
      voiceKaraokeAnchors[voiceIndex] = anchor;
    ...
    queuePlaybackTimeoutAt(() => {
      if (!isPlaying) return;
      for (const [voiceIndex, anchor] of firstAnchors) {
        startKaraokeLeadIn(voiceIndex, anchor, startsAtMs);
      }
    }, startsAtMs);
    return leadInMs;
  }
  ...
  async function startPlayback(readiness: AudioReadinessResult): Promise<void> {
    ...
    const totalCells = totalPlaybackCells();
    if (totalCells <= 0) {
      stopPlayback('no playback cells before start');
      return;
    }
    ...
    playbackPaused = false;
    const layoutSettled = await settlePlaybackLayout(startToken);
    if (!layoutSettled) return;

    const layoutTotalCells = totalPlaybackCells();
    if (layoutTotalCells <= 0) {
      stopPlayback('no playback cells after layout settled');
      return;
    }
    ...
    prepareHorizontalPlaybackHighway(currentIndex, traversalEndIndex);
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Horizontal playback start anchors resolved.', horizontalScrollDiagnosticsSnapshot({
        currentIndex,
        traversalEndIndex,
        shouldRunCountIn,
        startupDelayMs,
        ...
        resumingPlayback,
        firstAnchors: Array.from(firstAnchors.entries()).map(([voiceIndex, anchor]) => ({
          voiceIndex,
          anchor: karaokeAnchorDebugSnapshot(anchor),
        })),
        anchors: horizontalPlaybackAnchorDebugSnapshot(currentIndex, traversalEndIndex),
      }));
    }
    if (trackStyle === 'horizontal') {
      ...
      debugHorizontalScroll('Horizontal playback highway prepared for start.', horizontalScrollDiagnosticsSnapshot({
        currentIndex,
        traversalEndIndex,
        shouldUseHorizontalPlaybackHighway,
        anchors: horizontalPlaybackAnchorDebugSnapshot(currentIndex, traversalEndIndex),
    ...
    const leadInMs = firstAnchors.size > 0
      ? shouldRunCountIn
        ? startupDelayMs + countInBeatIntervalMs() * COUNT_IN_NUMBERS.length
        : shouldUseHorizontalPlaybackHighway
          ? startupDelayMs
          : startupDelayMs + karaokeLeadInDurationMs()
      : startupDelayMs;
    const timeline = schedulePlaybackAudioRun(currentIndex, traversalEndIndex, leadInMs, shouldRunCountIn, startupDelayMs);
    if (!timeline) {
      stopPlayback('audio scheduling failed');
      return;
    }

    activePlaybackTimeline = timeline;
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Horizontal playback timeline activated.', playbackTimelineDebugSnapshot(timeline, {
        currentIndex,
        traversalEndIndex,
        leadInMs,
        firstAnchorCount: firstAnchors.size,
    ...
    if (firstAnchors.size > 0) {
      if (shouldRunCountIn) {
        startCountIn(firstAnchors, currentIndex, timeline.transportStartAtMs, startupDelayMs);
      } else if (shouldUseHorizontalPlaybackHighway) {
        holdKaraokeAtFirstAnchors(firstAnchors, currentIndex);
      } else {
        startKaraokeLeadInAt(firstAnchors, timeline.transportStartAtMs + startupDelayMs);
      }
    }
    ...
    queuePlaybackTimeoutAt(() => {
      ...
      debugPlaybackStartup('Playback lead-in completed.', {
        leadInMs,
        actualDelayMs: Math.round(performance.now() - timeline.transportStartAtMs),
      });
      clearCountInDisplay();
    }, timeline.musicStartAtMs);
    startPlaybackVisualLoop(currentIndex, timeline.musicStartAtMs);
  }
  ...
  function pausePlayback(): void {
    ...
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Horizontal playback paused.', horizontalScrollDiagnosticsSnapshot({
        playbackIndex,
      }));
    }
  ...
  function stopPlayback(reason = 'unspecified'): void {
    const stopReason = reason.trim() || 'unspecified';
    debugPlaybackHighlight('Stopping playback.', { playbackIndex, reason: stopReason });
    if (trackStyle === 'horizontal') {
      debugHorizontalScroll('Horizontal playback stopped.', horizontalScrollDiagnosticsSnapshot({
        playbackIndex,
        reason: stopReason,
      }));
    }
  ...
  function handleStopPlaybackClick(): void {
    stopPlayback('stop button');
  }
  ...
  function handleGlobalKeyDown(event: KeyboardEvent): void {
    ...
    if (event.code === KEY_CODES.KEY_O) {
      event.preventDefault();
      stopPlayback('keyboard stop shortcut');
      return;
    }
    ...
    <div class="controls-group transport-group">
      <div class="transport-actions">
        <div class="transport-row">
          ...
          <button type="button" class="transport-btn stop-btn" on:click={handleStopPlaybackClick} title="Stop" aria-label="Stop">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          >
            ...
            >
              {#if trackStyle === 'horizontal' || voiceCount > 1}
                <button
                  type="button"
                  class="voice-track-row-label"
                  class:voice-track-row-label--playback-start={isPlaybackStartLabelSelected(sourceRowIndex)}
                  disabled={isPlaying}
                  aria-pressed={isPlaybackStartLabelSelected(sourceRowIndex)}
                  aria-label={`Start playback at measure ${sourceRowIndex + 1}`}
                  title={`Start playback at measure ${sourceRowIndex + 1}`}
                  on:pointerdown={(event) => handleMeasureLabelPointerDown(event, voiceIndex)}
                  on:click={(event) => handleMeasureLabelClick(event, voiceIndex, sourceRowIndex)}
                >{sourceRowIndex + 1}</button>
              {/if}
              {#if hasInlinePickup}
