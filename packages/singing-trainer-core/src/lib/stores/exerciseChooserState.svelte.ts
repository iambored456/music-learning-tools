/**
 * Exercise Chooser State - Svelte 5 Runes
 *
 * Manages the standalone exercise chooser modal visibility and selection.
 */

import type { LessonSettingsSchema } from '@mlt/lesson-templates';

export interface ExerciseChooserState {
  isVisible: boolean;
  selectedExerciseId: string | null;
  localSettings: Record<string, number | boolean>;
}

const DEFAULT_STATE: ExerciseChooserState = {
  isVisible: false,
  selectedExerciseId: null,
  localSettings: {},
};

function createExerciseChooserState() {
  let state = $state<ExerciseChooserState>({ ...DEFAULT_STATE });
...
export const exerciseChooserState = createExerciseChooserState();
