 ...
 * Harmonic Bins
 *
 * Main UI component for harmonic coefficient editing.
 * Provides slider controls for 12 harmonic bins with phase buttons.
 *
 * Sub-modules:
...
interface HarmonicBinsDebugMessage {
  level: DebugLevel;
  args: unknown[];
  timestamp: number;
}
...
const harmonicBinsDebugMessages: HarmonicBinsDebugMessage[] = [];

function recordHarmonicBinsDebug(level: DebugLevel, ...args: unknown[]) {
...
let harmonicBinsGrid: HTMLDivElement | null = null;
...
function drawFilterOverlay() {
  ...
  if (!overlayCanvas || !overlayCtx || !harmonicBinsGrid) {
    ...
    harmonicBinsGrid = document.querySelector<HTMLDivElement>('.harmonic-bins-grid');
    if (overlayCanvas) {
...
function handleBinPointerEvent(e: PointerEvent, binIndex: number | null = null) {
  if (!currentColor || !harmonicBinsGrid) {
    return;
  }
  ...
  logger.debug('HarmonicBins', 'handleBinPointerEvent called', { target: target?.className }, 'filter');
  ...
  if (clampedValue === 0) {
    ...
    logger.debug('HarmonicBins', 'BEFORE creating newCoeffs - store coeffs', { coeffs: timbre.coeffs }, 'filter');
    const newCoeffs = new Float32Array(timbre.coeffs);
...
export function initHarmonicBins() {
  ...
  harmonicBinsGrid.addEventListener('pointerdown', (e: PointerEvent) => {
    const target = e.target instanceof Element ? e.target : null;
    recordHarmonicBinsDebug('log', '[BINS DRAG] Pointerdown event fired', { target, className: target?.className });
    ...
    if (color && color === currentColor) {
      ...
      if (coeffs.length !== newCoeffs.length) {
        ...
        for (let i = 0; i < newCoeffs.length; i++) {
          ...
          if (diff > 0.001) {
            logger.debug('HarmonicBins', `Coefficient ${i} changed: ${localVal} -> ${storeVal} (diff: ${diff})`, null, 'filter');
            coeffsChanged = true;
            break;
          }
        }
